...
export default function WeeklyScreen() {
  ...
  return (
    <AppBackground>
      <ScreenHeader title="Weekly Goals" subtitle={`${weekly.weekId || getWeekId()} - Monday reset`} />
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Card style={styles.stack}>
          <Text variant="heading2">{complete ? 'Reward Ready' : 'This Week'}</Text>
          <Text variant="bodySmall">{weekly.rewardClaimed ? 'Weekly reward claimed.' : 'Complete all goals to earn hints and a booster.'}</Text>
        </Card>
        {weekly.objectives.map((objective) => (
          <Card key={objective.id} style={styles.stack}>
            <View style={styles.row}>
              <Text variant="title">{objective.title}</Text>
              <Text variant="caption">{objective.completed ? 'DONE' : `${objective.progress} / ${objective.target}`}</Text>
            </View>
